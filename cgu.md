# Conditions Générales d’Utilisation (CGU) / Terms of Use

_Last updated / Dernière mise à jour : 11 December 2025_

> This document contains both the English and French versions of the Terms of Use.  
> Ce document contient les versions anglaise et française des Conditions Générales d’Utilisation.

**Jump to / Aller vers :**  
➡️ [English Version](#english-version)  
➡️ [Version Française](#version-française)

---

## English Version

>Welcome to **Nox**, the application designed to help you recentre, meditate, and explore lucid dreaming in a peaceful environment.

These Terms of Use govern access to and usage of:

*   the **Nox** mobile application,
*   the website www.nox-app.com [www.nox-app.com](https://www.nox-app.com),

(hereinafter referred to as “the Service”).

By using Nox, you fully and unconditionally agree to these Terms of Use.

### 1. Purpose of the Terms of Use

These Terms of Use define:

* the conditions for accessing the Service,
* the rules governing its use,
* the rights and responsibilities of the user,
* as well as the obligations of the Service Provider.

### 2. Access to the Service

Accessing the Service requires:
* a device connected to the Internet,
* a stable network connection.

Basic access to the Service is free.

Some features are available exclusively through a *paid subscription* (premium content, personalized AI-generated meditations, etc.).

The Service is provided on a best-effort basis, without any guarantee of continuous availability.

### 3. Account Creation and Authentication

Certain features require the creation of a user account.

Registration is performed exclusively through **Apple Sign-In**, ensuring secure authentication.

Users agree to provide accurate, up-to-date information and to keep it maintained.

### 4. Description of the Service

Nox generates daily:
* personalized meditations,
* audio, text, or visual content,
* based on your stated preferences, emotions, or needs.

The Service uses generative AI technologies to create unique, personalized experiences without requiring proactive action from the user.

**Its sole goal: to give you a moment of calm, away from the noise.**

### 5. Intellectual Property

All elements available within Nox — including texts, sounds, visuals, interfaces, logos, graphic identities, and generated content — are protected by copyright laws.

They are the exclusive property of the Service Provider, unless otherwise stated.  
**Any reproduction, distribution, or unauthorized use is strictly prohibited.**

### 6. Personal Data

The data we collect is used solely to:
* personalize your experience,
* improve generated content,
* ensure the proper functioning of the Service,
* manage subscriptions and payments.

We comply with **GDPR** regulations and **App Store** policies.
For more information, please refer to our **Privacy Policy**.

### 7. Payments and Subscription

Some features and content require a paid subscription.

Payments are securely processed through the **Apple App Store**.

Key points :
* Subscriptions **renew automatically**, unless canceled.
* You may cancel at any time through your App Store account settings.
* Prices are clearly displayed before any subscription is confirmed.
* Refunds cannot be provided directly by the Service Provider and must be requested through Apple.

### 8. Liability

The use of the Service is under your sole responsibility.

Nox:
* **does not replace medical, psychological, or therapeutic advice**,
* is not a diagnostic or treatment tool.

The Service Provider cannot be held liable for:
* misuse of the Service,
* temporary unavailability,
* indirect damages.

### 9. Modification of the Terms

The Service Provider may modify these Terms of Use at any time.

In the event of an update:
* the new version will be published on the Website and within the Application;
* continued use of the Service constitutes acceptance of the updated Terms.

### 10. Governing Law

These Terms of Use are governed by **French law**.

In the event of a dispute, the competent court shall be that of the Service Provider’s registered office, unless legal provisions specify otherwise (particularly for EU consumers).

### 11. Contact

For any questions regarding the Terms of Use or the Service, you may contact us at:  
contact@nox-app.com

---

## Version Française

>Bienvenue sur **Nox**, l’application conçue pour vous offrir un moment de recentrage, de méditation et de rêve lucide.

Les présentes Conditions Générales d’Utilisation encadrent l’accès et l’usage :
* de l’Application mobile **Nox**,  
* du site web [www.nox-app.com](https://www.nox-app.com),  

(ci-après « le Service »).

En utilisant Nox, vous acceptez pleinement et entièrement ces CGU.

### 1. Objet des CGU

Les présentes CGU définissent :
* les modalités d’accès,  
* les règles d’utilisation,  
* les droits et responsabilités de l’utilisateur,  
* ainsi que les obligations de l’Éditeur du Service.

### 2. Accès au Service

L’accès au Service nécessite :
* un appareil connecté à Internet,  
* une connexion réseau stable.

L’accès de base est gratuit.

Certaines fonctionnalités sont proposées dans le cadre d’un **abonnement payant** (contenus premium, méditations IA personnalisées…).

Le Service est fourni dans la mesure du possible, sans garantie de disponibilité continue.

### 3. Création de compte et authentification

Certaines fonctionnalités nécessitent la création d’un compte utilisateur.

L’inscription s’effectue exclusivement via **Apple Sign-In**, assurant une authentification sécurisée.

L’utilisateur s’engage à fournir des informations exactes et à les maintenir à jour.

### 4. Description du Service

Nox génère quotidiennement :
* des méditations personnalisées,  
* des contenus audio, textes ou visuels,  
* basés sur vos préférences, émotions ou besoins exprimés.

Le Service utilise des technologies d’IA générative, sans intervention proactive de votre part.

**Objectif : vous offrir un moment de calme, loin du bruit.**

### 5. Propriété intellectuelle

L’ensemble des éléments présents sur Nox — textes, sons, visuels, interfaces, logo, identité graphique, contenus générés — est protégé par le droit d’auteur.

Ils sont la propriété exclusive de l’Éditeur, sauf mention contraire.  
**Toute reproduction, distribution ou utilisation non autorisée est strictement interdite.**

### 6. Données personnelles

Les données collectées sont utilisées uniquement pour :
* personnaliser l’expérience utilisateur,  
* améliorer les contenus générés,  
* assurer le bon fonctionnement du Service,  
* gérer les abonnements et paiements.

Nous respectons la réglementation **RGPD** et les règles de l’**App Store**.  
Consultez notre **Politique de Confidentialité** pour plus de détails.

### 7. Paiement et abonnement

Certaines fonctionnalités sont accessibles via un abonnement payant.

Les paiements sont gérés de manière sécurisée via la plateforme **Apple (App Store)**.

Caractéristiques :
* L’abonnement est **renouvelé automatiquement**, sauf annulation.  
* Vous pouvez résilier à tout moment via les paramètres de votre compte App Store.  
* Les prix sont affichés avant toute souscription.  
* Aucun remboursement n’est effectué par l’Éditeur : seuls les systèmes Apple gèrent la facturation.


### 8. Responsabilité

L’utilisation du Service relève de votre seule responsabilité.

L’application Nox :
* ne remplace **aucun avis médical, psychologique ou thérapeutique**,  
* n’est pas un outil de diagnostic ni de traitement.

L’Éditeur ne peut être tenu responsable :
* d’une mauvaise utilisation du Service,  
* d’une indisponibilité temporaire,  
* de dommages indirects.


### 9. Modifications des CGU

Les présentes CGU peuvent être modifiées à tout moment.

En cas de modification :
* la nouvelle version sera publiée sur le Site et dans l’Application,  
* l’utilisation continue du Service vaut acceptation des nouvelles CGU.


### 10. Droit applicable

Les présentes CGU sont régies par le **droit français**.

En cas de litige, la juridiction compétente est celle du siège social de l’Éditeur, sauf dispositions légales contraires (notamment pour les consommateurs européens).

### 11. Contact

Pour toute question concernant les CGU ou le Service :  
contact@nox-app.com
